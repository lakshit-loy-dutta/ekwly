const App = {
	...
	applyQuickSplit() {
		...
		const evenShare = this.utils.round2(item.qty / selectedPeople.length);
		...
		// Apply new even share
		selectedPeople.forEach((person) => {
			this.state.assignments[person][item.id] = evenShare;
		});
		...
		trackerDiv.innerHTML = this.state.items
			.map((item) => {
				...
				this.state.people.forEach(
					(p) => (claimed += this.state.assignments[p][item.id] || 0),
				);
