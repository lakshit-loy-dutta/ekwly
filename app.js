const App = {
    ...
    addItem() {
        ...
        if (isNaN(qty) || qty <= 0) return this.showToast("Valid quantity is required.", "error");
        if (isNaN(unitPrice) || unitPrice < 0) return this.showToast("Valid unit price is required.", "error");
        ...
        this.state.items.push({ 
            ...
            name, qty, unitPrice, tax, 
            totalBase, totalTax, totalWithTax 
        });
    ...
    addPerson() {
        ...
        if (!name) return this.showToast("Participant name cannot be empty.", "error");
        if (this.state.people.includes(name)) return this.showToast("Name must be unique.", "error");
    ...
    updateShare(person, itemId, value) {
        this.state.assignments[person][itemId] = parseFloat(value) || 0;
        this.renderTrackers(); 
    },

    // --- Quick Split Tool (Redesigned with Tap-Chips) ---
    renderQuickSplit() {
        ...
        select.innerHTML = this.state.items.map(item => 
        ...
        peopleDiv.innerHTML = this.state.people.map(person => `
    ...
    applyQuickSplit() {
        ...
        const activeChips = document.querySelectorAll('#qsPeople .toggle-chip.active');
        const selectedPeople = Array.from(activeChips).map(chip => chip.dataset.name);

        if (selectedPeople.length === 0) {
            return this.showToast("Select at least one participant.", "error");
        }

        // Reset assignment for this item across all people
        this.state.people.forEach(person => {
            this.state.assignments[person][item.id] = 0;
        });

        // Smart distribution algorithm to eliminate fractional remainders
        // e.g. 1 Qty / 3 People = [0.33, 0.34, 0.33] instead of [0.33, 0.33, 0.33 with 0.01 leftover]
        let runningAllocated = 0;
        selectedPeople.forEach((person, index) => {
            const targetTotal = this.utils.round2((item.qty / selectedPeople.length) * (index + 1));
            const thisShare = this.utils.round2(targetTotal - runningAllocated);
            this.state.assignments[person][item.id] = thisShare;
            runningAllocated = this.utils.round2(runningAllocated + thisShare);
        });
        ...
        this.renderTrackers();    
        this.showToast(`Divided ${item.name} evenly among ${selectedPeople.length} people.`);
    },
    ...
    renderItems() {
        ...
        tbody.innerHTML = this.state.items.map(item => `
            <tr>
                ...
                <td class="text-right">${(item.tax * 100).toFixed(0)}%</td>
    ...
    renderPeople() {
        ...
        div.innerHTML = this.state.people.map(p => `
            <div class="chip">
    ...
    renderTrackers() {
        ...
        trackerDiv.innerHTML = this.state.items.map(item => {
            let claimed = 0;
            this.state.people.forEach(p => claimed += (this.state.assignments[p][item.id] || 0));
    ...
    renderAssignments() {
        ...
        if (this.state.people.length === 0 || this.state.items.length === 0) {
            grid.innerHTML = '<div class="empty-state border-dashed w-full" style="grid-column: 1 / -1;">Add items and participants to begin splitting.</div>';
            return;
        }

        grid.innerHTML = this.state.people.map(person => `
                ...
                <div class="person-card-body">
                    ${this.state.items.map(item => `
                        <div class="item-row">
                            ...
                            <input type="number" min="0" step="0.01" value="${this.state.assignments[person][item.id] || ''}" 
    ...
    calculateSplit() {
        if (this.state.items.length === 0 || this.state.people.length === 0) {
            return this.showToast("Need items and participants to calculate.", "error");
        }
        ...
        this.state.items.forEach(item => {
            ...
            if (diff > 0) warnings.push(`'${item.name}' has ${diff} unassigned.`);
            if (diff < 0) warnings.push(`'${item.name}' is over-assigned by ${Math.abs(diff)}.`);
        });

        if (warnings.length > 0) {
            const proceed = confirm("Math Warning:\n" + warnings.join("\n") + "\n\nDo you want to force calculation anyway?");
            if (!proceed) return;
        }

        let globalSummary = { totalQty: 0, subTotal: 0, totalVAT: 0, totalGST: 0, serviceCharge: 0, grandTotal: 0 };
        
        this.state.items.forEach(item => {
            globalSummary.totalQty += item.qty;
            globalSummary.subTotal += item.totalBase;
            if (item.tax === 0.06) globalSummary.totalVAT += item.totalTax;
            else if (item.tax === 0.05) globalSummary.totalGST += item.totalTax;
        });
        ...
        this.state.people.forEach(person => {
            ...
            this.state.items.forEach(item => {
                const consumedQty = this.state.assignments[person][item.id] || 0;
                if (consumedQty > 0) {
                    const costShare = (consumedQty / item.qty) * item.totalWithTax;
                    subtotal += costShare;
                    consumedItems.push({ name: item.name, qty: consumedQty, cost: costShare });
                }
            });

            totalSubtotalForProportions += subtotal;
            individualBreakdowns[person] = { subtotal: subtotal, items: consumedItems };
        });
        ...
        Object.keys(individualBreakdowns).forEach(person => {
            ...
            const proportion = totalSubtotalForProportions > 0 ? (data.subtotal / totalSubtotalForProportions) : 0;
            const scAmount = (totalSubtotalForProportions * this.state.serviceChargeRate) * proportion;
    ...
    renderResults() {
        ...
        const { individualBreakdowns, globalSummary } = this.state.calculationResult;
        ...
        htmlStr += Object.keys(individualBreakdowns).map(person => {
                    ...
                    <div class="person-card-body">
                        ${b.items.map(i => `
                            <div class="result-item">
                                <span>${this.utils.escapeHTML(i.name)} <span class="text-sm text-muted">x${i.qty}</span></span>
    ...
    exportToPDF() {
        try {
            if (!this.state.calculationResult) return this.showToast("Calculate split first.", "error");
            if (!window.jspdf || !window.jspdf.jsPDF) return this.showToast("PDF Library failed to load.", "error");

            const { jsPDF } = window.jspdf;
            const doc = new jsPDF();
            
            const customTitle = document.getElementById('receiptTitle').value.trim();
            ...
            doc.text(`Generated by FairSplit Pro on ${dateStr}`, 14, startY + 6);
            ...
            const masterTableData = this.state.items.map(i => [
                i.name, i.qty.toString(), `Rs. ${i.unitPrice.toFixed(2)}`, `${(i.tax * 100).toFixed(0)}%`, `Rs. ${i.totalWithTax.toFixed(2)}`
            ]);

            doc.autoTable({
                startY: startY,
                head: [['Item', 'Qty', 'Unit Price', 'Tax', 'Total (inc. Tax)']],
                ...
                columnStyles: { 0: { fontStyle: 'bold' }, 1: { halign: 'center' }, 2: { halign: 'right' }, 3: { halign: 'center' }, 4: { halign: 'right', fontStyle: 'bold' } },
            ...
            if (globalSummary.totalGST > 0) summaryData.push(["Total GST (5%):", `Rs. ${globalSummary.totalGST.toFixed(2)}`]);
            if (globalSummary.totalVAT > 0) summaryData.push(["Total VAT (6%):", `Rs. ${globalSummary.totalVAT.toFixed(2)}`]);
            if (globalSummary.serviceCharge > 0) summaryData.push(["Total Service Charge:", `Rs. ${globalSummary.serviceCharge.toFixed(2)}`]);
            
            summaryData.push([
                { content: "Grand Total:", styles: { fontStyle: 'bold', textColor: primaryColor, fontSize: 13 } }, 
                { content: `Rs. ${globalSummary.grandTotal.toFixed(2)}`, styles: { fontStyle: 'bold', textColor: primaryColor, fontSize: 13 } }
            ]);
            ...
            const { individualBreakdowns, scRate } = this.state.calculationResult;
            ...
            Object.keys(individualBreakdowns).forEach(person => {
                ...
                const tableData = b.items.map(i => [ i.name, i.qty.toString(), `Rs. ${i.cost.toFixed(2)}` ]);

                if (this.state.serviceChargeRate > 0) {
                    tableData.push([{ content: "Service Charge Proportion", styles: { textColor: mutedText } }, "-", { content: `Rs. ${b.serviceCharge.toFixed(2)}`, styles: { textColor: mutedText } }]);
                }
                
                tableData.push([
                    { content: "Owes:", colSpan: 2, styles: { halign: 'right', fontStyle: 'bold', fillColor: [243, 244, 246], textColor: primaryColor, fontSize: 11 } }, 
                    { content: `Rs. ${b.totalOwed.toFixed(2)}`, styles: { fontStyle: 'bold', fillColor: [243, 244, 246], textColor: primaryColor, fontSize: 11 } }
                ]);
                ...
                doc.autoTable({
                    ...
                    body: tableData,
                    theme: 'grid',
                    headStyles: { fillColor: [243, 244, 246], textColor: mutedText, fontStyle: 'normal' }, 
            ...
            const safeFilename = customTitle ? customTitle.replace(/[^a-z0-9]/gi, '_').toLowerCase() : `Settlement_${new Date().toISOString().slice(0,10)}`;
